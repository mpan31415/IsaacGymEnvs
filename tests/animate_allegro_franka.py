...
# set the default positions for the two Franka arms to home
left_franka_home_pos = [math.pi/8, -math.pi/4, 0.0, -7*math.pi/8, 0.0, 7*math.pi/8, -math.pi/4]
right_franka_home_pos = [-math.pi/8, -math.pi/4, 0.0, -7*math.pi/8, 0.0, 7*math.pi/8, 3*math.pi/4]
dof_positions[12:19] = left_franka_home_pos
dof_positions[35:42] = right_franka_home_pos
...
# position the camera
cam_pos = gymapi.Vec3(18.2, 2.0, 16)
...
while not gym.query_viewer_has_closed(viewer):
    ...
    if MOVE:
        ...
        if anim_state == ANIM_SEEK_DEFAULT:
            ...
            current_dof = start_anim_dof + ((current_dof - start_anim_dof + 1) % (num_dofs - start_anim_dof))
            anim_state = ANIM_SEEK_LOWER
            print("Animating DOF %d ('%s')" % (current_dof, dof_names[current_dof]))
